                ...
                <div class="card-body">
                    <table class="table aiz-table mb-0">
                        <tr>
                            ...
                            <td>{{ count(\App\Models\OrderDetail::where('seller_id', Auth::user()->id)->get()) }}</strong>
                            </td>
                        </tr>
                        <tr>
                            <td>{{ translate('Pending orders') }}:</td>
                            <td>{{ count(\App\Models\OrderDetail::where('seller_id', Auth::user()->id)->where('delivery_status', 'pending')->get()) }}</strong>
                            </td>
                        </tr>
                        <tr>
                            <td>{{ translate('Cancelled orders') }}:</td>
                            <td>{{ count(\App\Models\OrderDetail::where('seller_id', Auth::user()->id)->where('delivery_status', 'cancelled')->get()) }}</strong>
                            </td>
                        </tr>
                        <tr>
                            <td>{{ translate('Successful orders') }}:</td>
                            <td>{{ count(\App\Models\OrderDetail::where('seller_id', Auth::user()->id)->where('delivery_status', 'delivered')->get()) }}</strong>
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body text-center">
                        @if (Auth::user()->seller->seller_package)
                            <img src="{{ uploaded_asset(Auth::user()->seller->seller_package->logo) }}"
                                class="img-fluid mb-4 h-110px">
                            <p class="mb-1 text-muted">{{ translate('Product Upload Limit') }}:
                                {{ Auth::user()->seller->seller_package->product_upload_limit }}
                                {{ translate('Times') }}</p>
